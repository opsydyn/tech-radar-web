...
const isOverlapping = (
  ...
  getDistance(blip1.position, blip2.position) < minDistance;
...
const findNonOverlappingPosition = (
  ...
  placedBlips: ReadonlyArray<BlipWithPosition>,
  centerX: number,
  centerY: number,
  minDistance: number,
  ...
  const adjustmentStrategies = {
    ...
    combined: (i: number): [number, number] => [(0.1 + (i * 0.02)) * 0.7, (5 + i) * 0.7]
  };

  // Pure function to get strategy key based on index
  const getStrategyKey = (index: number): keyof typeof adjustmentStrategies => {
    const keys = Object.keys(adjustmentStrategies) as Array<keyof typeof adjustmentStrategies>;
  ...
  function* createAdjustmentSequence(): Generator<[number, number]> {
    ...
    function* yieldAdjustments(index: number): Generator<[number, number]> {
      ...
      yield* yieldAdjustments(index + 2);
    }
    
    yield* yieldAdjustments(15);
  }
  ...
  return adjustments.reduce(
    (currentBlip, [angleAdjustment, radiusAdjustment]) => {
      // If we already found a non-overlapping position, return it
      if (hasNoOverlaps(currentBlip)) {
        return currentBlip;
      }

      // Create a new position with the current adjustments
...
const useBlipPositions = (blips: Array<Blip>) => {
  ...
  const calculatePolarCoordinates = useCallback(
    (
      ...
      const angle = quadrantBaseAngle + anglePerBlip * (blipIndexInQuadrantAndRing + 1);
      ...
      const blipRadius = innerRadius + (outerRadius - innerRadius) * positionFactor;
  ...
  return useMemo(() => {
    ...
    const groupBlipsByQuadrantAndRing = (blips: Array<Blip>): Map<string, Array<Blip>> => {
      return blips.reduce((groups, blip) => {
        ...
        return groups;
      }, new Map<string, Array<Blip>>());
    };

    const blipGroups = groupBlipsByQuadrantAndRing(blips);

    // Calculate initial positions
    const initialPositions: Array<BlipWithPosition> = blips.map(blip => {
      const key = `${blip.quadrant}-${blip.ring}`;
    ...
    const sortByRingPriority = (blips: Array<BlipWithPosition>): Array<BlipWithPosition> => {
      const ringPriority = {
        'Adopt': 0,
        'Trial': 1,
        'Assess': 2,
      ...
      return [...blips].sort((a, b) =>
        (ringPriority[a.ring] ?? defaultPriority) - (ringPriority[b.ring] ?? defaultPriority)
      );
    };
    ...
    const adjustPositions = (
      blips: Array<BlipWithPosition>
    ): Array<BlipWithPosition> => {
      const result: Array<BlipWithPosition> = [];
      ...
      for (const blip of blips) {
        ...
        const { angle, radius: currentRadius } = cartesianToPolar(adjustedBlip.position);
        ...
        const getRadiusStrategy = (): keyof typeof radiusAdjustmentStrategies => {
